...
export default class BenzAMRRecorder {
  ...
  initWithBlob(blob) {
    ...
    return p.then(data => this.initWithArrayBuffer(data));
  }
  ...
  initWithUrl(url) {
    ...
    return p.then(array => this.initWithArrayBuffer(array));
  }
  ...
  static rawAMRData2Blob(data) {
    return new window.Blob([data.buffer], {
      type: 'audio/amr',
    });
  }
}
